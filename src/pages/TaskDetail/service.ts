...
export async function flowByTask(body: API_Detail.flowListByTask) {
    if (body.Model == 1) {
        return request<API_Detail.ueFlowList>(`/myapi/timeflow/getTimeFlowByTaskId?taskId=${body.TaskID}`, {
            method: 'GET',
        });
    }
    else {
...
export async function packetByueid(body: API_Detail.packetListByFlow) {
    return request<API_Detail.packetList>(`/myapi/packet/getPacketByFlowId?flowId=${body.FlowId}&model=${body.Model}`, {
        method: 'GET',
    });
}
